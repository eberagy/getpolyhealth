...
export async function POST(request: Request) {
  try {
    ...
    const submission = createDemoRequest({
      name,
      email,
      clinicName,
      specialty,
    ...
    try {
      ...
      console.error("Failed to deliver demo request.", deliveryError);

      return NextResponse.json(
        {
          error:
            "We couldn't send your request right now. Please email founder@telemd.app.",
        },
        { status: 500 },
      );
    }
    ...
    console.error("Unexpected demo request failure.", error);
